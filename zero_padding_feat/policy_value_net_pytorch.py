# -*- coding: utf-8 -*-
"""
Policy-Value Network with Zero Padding edge detection feature.
Modified: Input channels changed from 4 to 5 for edge marker feature plane.
"""
...
class Net(nn.Module):
    """Policy-Value Network with ResNet architecture"""
    def __init__(self, board_width, board_height, num_channels=128, num_res_blocks=6):
        ...
        # Modified: Input channels changed from 4 to 5 for Zero Padding edge markers
        self.conv_initial = nn.Conv2d(5, num_channels, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x_policy = x_policy.view(-1, 4 * self.board_width * self.board_height)
        x_policy = F.log_softmax(self.policy_fc(x_policy), dim=1)

        # Value head
        x_value = F.relu(self.value_bn(self.value_conv(x)))
        x_value = x_value.view(-1, 2 * self.board_width * self.board_height)
...
class PolicyValueNet():
    ...
    def policy_value_fn(self, board):
        ...
        # Modified: reshape uses 5 channels instead of 4 for Zero Padding
